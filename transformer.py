...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float) -> None:
        ...
        position = torch.arange(0, seq_len, dtype=torch.float).unsqueeze(1) #[dim, 1]
        ...
        self.register_buffer('pe', pe)  # this wont be a normal tensor and will be stored only on the cpu

    def forward(self, x):
        #x's first dimension has sequence_length data which we exactly need to splice pe
        x = x + (self.pe[:, :x.shape[1], :]).requires_grad_(False) # Broadcasting 0 dim such that all batches can use the same (1,seq_length, d_model) positional embedding !
...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, eps: float = 10**-6) -> None: # -> None is a type hint
        ...
        self.bias = nn.Parameter(torch.zeros(1)) # This param is added and above mul
...
class MultiHeadAttentionBlock(nn.Module):
    def __init__(self, d_model: int, h: int, dropout: float) -> None:
        ...
        self.d_model = d_model
        self.h = h
        assert d_model % h == 0, "d_model should be divisible by number of heads needed"
    ...
    def forward(self, q, k, v, mask):
        query = self.w_q(q) # [batch, n_tokens, embed]
        key  = self.w_k(k)
        value = self.w_v(v)

        query = query.view(query.shape[0], query.shape[1], self.h, self.d_k).transpose(1,2) # h * d_k = d_model , basically you have h heads
        # use transpose if you want swap dims, and use permute if you want to swap multiple dims; ex: .permute(0,3,2,1) -> let og be [0,123]
...
class ProjectionLayer(nn.Module):
    ...
    def forward(self, x):
        # (batch, n tokens, d_model) => (batch, n tokens, vocab_size);
        return torch.log_softmax(self.proj(x), dim=-1) # softmax along the vocab_size
